...
$ref = $_GET['reference'];
		...
		header("location:  javascript://history.go(-1)");
					...
					$query_cert_table  ="SELECT * FROM `clone_invest` WHERE `clone_invest`.`invoice_no`='$ref'";
...
if($result->data->status == 'success'){
   ...
   $date_time = date('Y-m-d');
			...
			$query_cert_table  ="SELECT * FROM `clone_invest` WHERE `clone_invest`.`invoice_no`='$reference'";
			$query_certs_result=mysqli_query($homedb,$query_cert_table); 
			while($row = mysqli_fetch_array($query_certs_result))
			{
		...
		$queryupdate=("UPDATE `clone_invest` SET
		...
		`approval_officer`  = '".mysqli_real_escape_string($homedb, 'Managment')."',
		`date_approved`     = '".mysqli_real_escape_string($homedb, $date_time)."'
		WHERE `clone_invest`.`invoice_no`='$reference'");
